...
import { GET } from 'src/app/api/groupSaving/[groupId]/contribution/route';
...
describe('Test GET contribution', () => {
  ...
  it('success: admin + with search params', async () => {
    ...
    searchParams = `?userId=${userId}&fromDate=${from}&toDate=${to}`;
  ...
  it('failed: should return 500 if error occurs', async () => {
    ...
    expect(json.response).toBe('Failed to get contribution: ' + error);
  });
